...
def test_prune_after_layer_k_reduces_sequence_keeps_original_positions():
    from vtp_eval.proposed_method import stage2_llm
    from vtp_eval.proposed_method.config import ProposedConfig
    B, H, S, D = 1, 2, 7, 8
    ...
    h2, pos2, mask2 = stage2_llm.prune_after_layer_k(hidden, attn, spans, cfg)
    # keep = non-vision {0,1,5,6} + top-1 vision {3} = 5 tokens
    assert h2.shape == (B, 5, D)
    # NO-SLICE: kept tokens retain their ORIGINAL (gapped) positions, not contiguous.
    assert pos2.tolist() == [[0, 1, 3, 5, 6]]
    assert mask2.shape == (B, 1, 5, 5)               # fresh causal mask over kept len
