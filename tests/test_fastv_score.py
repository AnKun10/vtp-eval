...
def test_uses_only_last_row_over_vision_slice():
    # attn: [B=1, H=1, S=4, S=4]; vision tokens at columns 1..3 (vision idx 0..2).
    attn = torch.zeros(1, 1, 4, 4)
    attn[0, 0, 3, 1] = 0.2   # last token -> vision[0]
    attn[0, 0, 3, 2] = 0.9   # last token -> vision[1]  (highest in the LAST row)
    attn[0, 0, 3, 3] = 0.1   # last token -> vision[2]
    # Poison in an EARLIER row, in a DIFFERENT vision column (vision[0]); a buggy
    # all-rows mean would make vision[0] win (argmax 0). Last-row-only -> argmax 1.
    attn[0, 0, 0, 1] = 5.0
